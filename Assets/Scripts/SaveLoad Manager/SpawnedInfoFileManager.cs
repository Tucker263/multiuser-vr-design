...
// BaseInfo派生クラスのセーブ・ロードクラス、生成オブジェクト
public static class SpawnedInfoFileManager
{
    ...
    public static void Load(string directoryPath, TagID tagID, Type infoType)
    {
        ...
        foreach (var jsonData in jsonList)
        {
            ...
            //ネットワークオブジェクト化
            GameObject obj = PhotonNetwork.Instantiate(info.name, info.position, info.rotation);
            if (obj == null)
            {
